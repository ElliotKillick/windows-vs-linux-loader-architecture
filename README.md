...
- Backward compatibility
  ...
  - With concurrency, this means a programmer must be conscientious not to break any assumptions applications may make about thread synchronization, the order of operations, and related state
    ...
    - Maintaining backward compatibility with concurrent systems amplifies in difficulty when some Windows DLLs publicly expose their locks (e.g. NTDLL exposes loader lock through the exported `LdrLockLoaderLock`/`LdrUnlockLoaderLock` functions and MSVCRT exposes all its locks through the exported `_lock` and `_unlock` functions) largely as hacks to use for working around poor concurrent system design or other potentially incorrect usages (glibc doesn't partake in exposing these private implementation details)
