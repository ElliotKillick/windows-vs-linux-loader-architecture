...
The Windows loader, in contrast to the GNU loader (or Unix-like loaders in general), is more vulnerable to deadlock or crash scenarios for a variety of architectural reasons. "The Root of `DllMain` Problems" (or more casually, "`DllMain` Rules Rewritten") provides a fundamental understanding of `DllMain` hurdles and why they exist. It improves on Microsoft's ["`DllMain` Best Practices"](https://learn.microsoft.com/en-us/windows/win32/dlls/dynamic-link-library-best-practices#general-best-practices), often referred to as the ["`DllMain` rules"](https://stackoverflow.com/a/4406638) informally. "`DllMain` Best Practices" originally goes back to an ancient (in technology years) [2006 Microsoft document](windows/dll-best-practices) (the document has undergone minimal changes on Microsoft's documentation website since then). The architectural reasons for `DllMain` issues in Windows include:
...
- Windows architecture prioritizes [libraries that provide functionality](https://github.com/wine-mirror/wine/tree/master/dlls), [processes housing multiple threads](https://github.com/ElliotKillick/windows-vs-linux-loader-architecture#case-study-on-abba-deadlock-between-the-loader-lock-and-gdi-lock), and [local/remote procedure calls (LPC/RPC)](https://github.com/reactos/reactos/blob/f10d40f9122b926bf01b5409a6d3c3d9d06806c3/dll/win32/kernel32/client/dllmain.c#L138) over separate processes
  - Splitting the core Windows API, C runtime, and all of the core system functionality across many DLLs (there's 3000+ DLLs in `C:\Windows\System32`) leaves more opportunities for actions done during library loading/unloading or initialization/deinitialization to go wrong
    - Microsoft built the loader to be reentrant, however, how the Windows loader performed module initialization for a long time was subject to crashes and other issues due to the loader's poor ability to enforce the correct order of operations when initializing modules or [even initialzing modules at all](https://learn.microsoft.com/en-us/archive/blogs/mgrier/the-nt-dll-loader-dll_process_attach-reentrancy-step-2-getprocaddress) upon being reentered (this issue was at the heart of the previous "`DllMain` Best Practices")
      - Microsoft introducing the dependency graph starting with Windows 8 [*significantly* helps solve the out-of-order module initialization problem](https://github.com/ElliotKillick/windows-vs-linux-loader-architecture#windows-loader-initialization-locking-requirements) even when loading a library from `DllMain` (i.e. `LoadLibrary` from `DllMain`), but there could potentially still be issues due to circular dependencies (["dependency loops"](https://learn.microsoft.com/en-us/windows/win32/dlls/dllmain#remarks))
      - The legacy loader effectively had a ["dependency graph"](https://learn.microsoft.com/en-us/archive/blogs/mgrier/the-nt-dll-loader-dll_process_attach-reentrancy-step-1-loadlibrary), however it was [only formed by walking the import address tables (IATs)](https://github.com/reactos/reactos/blob/513f3d179cff234821c359db034409e94a278320/dll/ntdll/ldr/ldrpe.c#L369) at the start of a load and [wasn't able to readjust dynamically](https://github.com/reactos/reactos/blob/513f3d179cff234821c359db034409e94a278320/dll/ntdll/ldr/ldrinit.c#L694) to the reentrant case of someone calling `LoadLibrary` from `DllMain`
    - A module you depend on or call a function from may have already deinitialized by the time your module deinitializes (i.e. the loader calls your module's `DLL_PROCESS_DETACH`)
    - [`GetModuleHandle` then `GetProcAddress` from `DllMain`](https://github.com/ElliotKillick/windows-vs-linux-loader-architecture#case-study) assuming a DLL is already loaded when it may not be yet or has only partially loaded thus far could cause problems (since the dependency graph won't pick up on a dynamic operation such as `GetModuleHandle`)
      - Nor POSIX or GNU loader extensions define a function like `GetModuleHandle`, instead only having `dlopen` due generally to the former function's inherent weaknesses
...
- Unexpectedly loading libraries
  - Inherently, delay loading may unexpectedly cause library loading when the programmer didn't intend, thus leading to deadlock
    - [MacOS previously supported lazy loading until removing it](https://github.com/ElliotKillick/windows-vs-linux-loader-architecture#lazy-loading-and-lazy-linking-os-comparison) likely due to scenarios where it becomes an anti-feature and any performance gains not being worth the trade-off
- Backward compatibility
  ...
  - With concurrency, this means you must be incredibly careful not to break any assumptions applications may make about thread synchronization, the order of operations, and related state
    ...
    - This fact rings especially true when DLLs like `ntdll.dll` put a pointer to some of its locks, such as loader lock, in the publicly accessible PEB or `msvcrt.dll` makes the `_lock` and `_unlock` functions a part of its exported API; even if there is a leading underscore marking it as an implementation detail (the new UCRT `ucrtbase.dll` DLL introduced with Windows 10 doesn't expose this functionality)
...
With your newfound understanding of problems affecting `DllMain` and the greater perspective gained from how Unix-like operating systems do it right, you can reason about the safety of performing a given action from `Dllmain` yourself.
...
When loading a library, loader work begins with the requesting thread calling `LdrpDrainWorkQueue` to perform mapping and snapping (i.e. both together is "processing"). By calling `LdrpDrainWorkQueue`, the thread requesting a library load agrees to become the load owner (`TEB.LoadOwner`). `LdrpDrainWorkQueue` increments `ntdll!LdrpWorkInProgress` and does the necessary mapping and snapping work, offloading this work to the parallel loader worker threads (`LoaderWorker` flag in `TEB.SameTebFlags`) whenever possible. Every increment to this reference counter past the initial increment from `0` to `1` by `LdrpDrainWorkQueue` indicates another loader worker thread processing a work item in parallel. While `ntdll!LdrpWorkInProgress` equals `1`, the last thing `LdrpDrainWorkQueue` does before returning is set the current thread as the load owner. With mapping and snapping complete, it's the load owner's job to fulfill the remainder of the loading process. The loader only uses the `ntdll!LdrpWorkQueue` linked list data structure is for enqueuing mapping and snapping work to the work queue. For all other operations, the loader primarily relies on the `Dependencies` and `IncomingDependencies` module information DAG data structures to navigate modules. From `LdrpDrainWorkQueue` to `LdrpDropLastInProgressCount` (where `ntdll!LdrpWorkInProgress` is finally set to zero and the current thread is decommissioned as the load owner) ties the entire loading process together. For more information, see the **[full reverse engineering of the `LdrpDrainWorkQueue` function](https://github.com/ElliotKillick/windows-vs-linux-loader-architecture#ldrpdrainworkqueue)**.
 ...
 <table summary="All LDR_DDAG_NODE.LDR_DDAG_STATE states, the function(s) responsible for each state change, and more information">
  ...
  <tr>
    ...
    <td>This state is set immediately before this function calls <code>LdrpSendPostSnapNotifications</code> to run post-snap DLL notification callbacks. The module is mapped and snapped but pending initialization. As the loader initializes nodes (i.e. modules) in the dependency graph (loader lock is held), each node's state will transition to <code>LdrModulesInitializing</code> then <code>LdrModulesReadyToRun</code>.</td>
  </tr>
  <tr>
...
Note that the solutions I provide above are mostly thought experiments. Due to [Windows' architectural issues](https://github.com/ElliotKillick/windows-vs-linux-loader-architecture#the-root-of-dllmain-problems), I can only offer bandage patches.
